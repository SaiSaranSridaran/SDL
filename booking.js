...
let selTime  = null;
let selSeats = [];
let allMovies = []; // will hold the loaded movie list

// ============================================================
//  LOAD BOOKED SEATS
// ============================================================
function loadBookedSeats() {
  const bookings = JSON.parse(localStorage.getItem('cvBookings')) || [];
  TAKEN.length = 0; // Clear array
  bookings.forEach(booking => {
    if (booking.seats) {
      const seatArray = booking.seats.split(',');
      seatArray.forEach(seat => {
        if (!TAKEN.includes(seat.trim())) {
          TAKEN.push(seat.trim());
        }
      });
    }
  });
...
function checkURLParam() {
  ...
  if (!id) return;
  // Try from allMovies first, then MOVIES fallback
  const movie = allMovies.find(m => m.id === id) || MOVIES.find(m => m.id === id);
  if (movie) {
    selMovie = movie;
...
function renderMovieSelect() {
  ...
  if (!grid) return;
  const movies = allMovies.length ? allMovies : MOVIES;
  grid.innerHTML = movies.map(m => `
    <div class="movie-select-card" data-id="${m.id}" onclick="pickMovie('${m.id}', this)">
      ...
      <p style="margin-top:4px;color:var(--gold);font-size:.75rem">₹${parseFloat(m.price).toFixed(0)} / seat</p>
    </div>`).join('');
}
  ...
  map.innerHTML = ROWS.map(row =>
    `<div class="seat-row">
      ...
      ${Array.from({ length: COLS }, (_, i) => {
        ...
        return `<div class="seat${taken ? ' taken' : ''}" data-id="${id}"
...
function buildConfirmCard() {
  ...
  const total = (selSeats.length * parseFloat(selMovie.price)).toFixed(2);
  card.innerHTML = `
    ...
    <div class="confirm-row"><span>Price per seat</span><span>₹${parseFloat(selMovie.price).toFixed(0)}</span></div>
...
function goToStep(n) {
  ...
  if (n === 2) {
    ...
    // Reset seat selection when going back to step 2
    selSeats = [];
    renderSeatMap();
    const disp = document.getElementById('selectedSeatsDisplay');
    if (disp) disp.textContent = 'None';
    const btn = document.getElementById('continueToConfirm');
...
async function submitBooking() {
  ...
  if (!selMovie || !selDate || !selTime || selSeats.length === 0) {
  ...
  const payBtn = document.querySelector('#step3 .btn-primary[onclick="submitBooking()"]');
  ...
  if (!data._offline && data.success) {
    ...
    const localBooking = { ...payload, id: Date.now(), time: selTime, status: 'confirmed' };
