...
function hasBookedMovie(movieId) {
  const local = JSON.parse(localStorage.getItem('cvBookings')) || [];
  const userId = currentUser?.id || currentUser?.email;
  return local.some(b =>
    (b.movie_id === movieId || b.movie === movieId) &&
    (b.user_id === userId || b.email === currentUser?.email)
  );
}
...
function openRatingModal(movieId) {
  ...
  if (!hasBookedMovie(movieId)) {
    showToast('You can only rate movies you have booked! 🎟️', 'error');
    return;
  }
  ...
  const initStars = existing ? existing.stars : 0;
  document.getElementById('ratingValue').value = initStars;
  document.getElementById('ratingReview').value = existing?.review || '';
  document.getElementById('ratingText').textContent = initStars ? initStars + ' out of 5 stars' : 'Click stars to rate';

  const starContainer = document.getElementById('ratingStars');
  starContainer.innerHTML = [1,2,3,4,5].map(n =>
    `<span data-star="${n}" style="transition:transform 0.1s;display:inline-block;"
      onclick="setRating(${n})"
...
function hoverStars(n) {
  const spans = document.getElementById('ratingStars').querySelectorAll('span');
  spans.forEach((s, i) => {
    s.textContent = i < n ? '★' : '☆';
...
function resetStarHover() {
  const locked = parseInt(document.getElementById('ratingValue').value) || 0;
  const spans = document.getElementById('ratingStars').querySelectorAll('span');
  spans.forEach((s, i) => {
    s.textContent = i < locked ? '★' : '☆';
    s.style.color = i < locked ? 'var(--gold)' : '';
...
function setRating(stars) {
  ...
  document.getElementById('ratingText').textContent = labels[stars] + ' — ' + stars + '/5 stars';
  resetStarHover();
}
...
async function submitRating() {
  ...
  const existingIdx = ratings.findIndex(r => r.movieId === currentRatingMovieId && r.userId === rating.userId);
...
function getUserRating(movieId) {
  ...
  const ratings = JSON.parse(localStorage.getItem('cvRatings')) || [];
  return ratings.find(r => r.movieId === movieId && r.userId === (currentUser.id || currentUser.email));
}
...
function getAverageRating(movieId) {
  ...
  const avg = movieRatings.reduce((sum, r) => sum + r.stars, 0) / movieRatings.length;
